...
class UserService:
    ...
    def create_user(self, data: CreateUserDTO) -> User:
        ...
        return User(
            ...
            password=self.__hash_password(data.password, data.username),
    ...
    def is_user_password_valid(self, user: User, password: str) -> bool:
        return user.password == self.__hash_password(password, user.username)

    def __hash_password(self, password: str, username: str) -> HashedPassword:
        data_to_hash = f"{password}{username}"
